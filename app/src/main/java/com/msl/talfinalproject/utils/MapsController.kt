package com.msl.talfinalproject.utils
...
import com.msl.talfinalproject.network.api.trashwise.models.Spot
...
class MapsController(context: Context, googleMap: GoogleMap) {
    ...
    fun setCustomMarker() {
        val blackMarkerIcon: BitmapDescriptor = BitmapDescriptorFactory.fromResource(R.drawable.ic_blue_marker)
        val markerOptions: MarkerOptions = MarkerOptions().position(mTimeSquare).title(mContext.getString(R.string.source)).snippet(mContext.getString(R.string.destination)).icon(blackMarkerIcon)
